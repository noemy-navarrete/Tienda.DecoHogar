...
function actualizarContadorMenu() {
  const enlaceCarrito = document.querySelector(".cart-container a");
  if (enlaceCarrito) {
    enlaceCarrito.textContent = `🛒 Cart (${carrito.length})`;
  }
}
...
function renderizarCarrito() {
  ...
  carrito.forEach((prod, index) => {
    ...
    itemDiv.innerHTML = `
      ...
      <button type="button" class="btn-eliminar-item" onclick="eliminarDelCarrito(${index})">
        ✕
      </button>
    `;
...
function agregarDireccion() {
  ...
  nuevaTarjeta.innerHTML = `
    ...
    <button type="button" class="btn-eliminar-dir">
